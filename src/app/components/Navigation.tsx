...
const tabs = [
  ...
  { id: "visarmature", label: "VisArmature" },
  { id: "visclay", label: "VisClay" },
  { id: "study", label: "User Study" },
    ...
    <nav className="bg-slate-800 sticky top-0 z-50">
      <div className="px-4 md:px-6">
        <div className="flex items-center h-16 gap-4 md:gap-12">
          ...
          {/* Desktop tabs - eight tabs and the title only fit side by side from xl up */}
          <div className="hidden xl:flex gap-1 ml-auto">
            {tabs.map((tab) => (
              <button
                key={tab.id}
          ...
          {/* Mobile hamburger */}
          <button
            className="xl:hidden ml-auto flex flex-col gap-1.5 p-2"
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="xl:hidden bg-slate-900 border-t border-slate-700">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => handleTabChange(tab.id)}
