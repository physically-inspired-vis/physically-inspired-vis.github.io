...
// A prototype's screenshot with a button beneath it that opens its tab. The
// button switches tabs in place and starts that page from the top, rather
// than at the far-down scroll position it is clicked from.
function ToolShowcase({ id, label, image, onTabChange }: {
  id: string;
  label: string;
  image: string;
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      ...
      <a
        ...
        className="inline-flex items-center gap-2 px-8 py-4 bg-zinc-950 hover:bg-zinc-800 text-white rounded-lg text-base font-medium transition-colors"
      >
        {label}
    ...
    <div className="overflow-auto">
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 md:py-12 flex flex-col gap-12 md:gap-20">
        ...
        <Section>
          ...
          <ArticleImage src="resources/examples stripe.png" alt="Examples of physically-inspired visualizations" width="100%" />
          <Body>
            Physically-inspired visualizations are data visualizations that draw on cues from the physical world — such as materials, lighting, depth, deformation, or physically suggestive motion — to communicate data. These cues can function as encodings, provide context, or shape interpretation. To better understand this growing visual vocabulary, we developed a design space grounded in a curated corpus of 103 examples across different media and production methods.
          </Body>
          <Body>
        ...
        {/* Worked example */}
        <Section>
          <SectionTitle>Using the design space to describe visualizations</SectionTitle>
          <Body>
            Here is an example of applying the design space to “No Way, Norway,” which illustrates the number of whales caught by country in 2015. You can see the fully coded example{" "}
            <a
              href="https://physically-inspired-vis.github.io/coder/?example=004"
              target="_blank"
              rel="noopener noreferrer"
              className="underline hover:text-foreground transition-colors"
            >
          ...
          <ArticleImage src="resources/coded_example.png" alt="The design space applied to “No Way, Norway”" width="100%" />
        </Section>

        {/* Operationalization */}
        <Section>
          <SectionTitle>Operationalization</SectionTitle>
          <Body>
            We operationalized the design space in two prototype authoring tools. VisArmature focuses on structure and encoding. It supports building physically-inspired visualizations from marks and nested collections whose physical attributes are mapped to data.
          </Body>
          <ToolShowcase id="visarmature" label="VisArmature" image="resources/visarmature.png" onTabChange={onTabChange} />
          <div className="mt-8">
            <Body>
              VisClay focuses on physical transformations and mechanisms, using effects such as stretching, shattering, and particle behaviors as data encodings.
            </Body>
          </div>
          <ToolShowcase id="visclay" label="VisClay" image="resources/probe2.png" onTabChange={onTabChange} />
        </Section>
